...
- 首页默认展示 PPT。Stage 1 左侧为横向 `image001-acn.png`；其他阶段直接使用该分支原三栏布局中的左栏，置于竖向区域。
...
`image001.png` 保留用户提供的 905×588 原图。页面使用内置图片编辑工具将中心文字 NGC 改为 ACN 的 `image001-acn.png`（1556×1011），等比显示并通过 CSS 裁去边缘选框点。编辑提示词记录在 `image001-acn.edit.json`；素材来源及阶段列表记录在 `source.json`。已确认的 ACN 图片随代码保存，重新生成 PPT 素材不会覆盖它。
