...
- 首页默认展示 PPT。Stage 1 左侧为横向场景图和独立的矢量文字层；其他阶段直接使用该分支原三栏布局中的左栏，置于竖向区域。
...
`image001.png` 保留用户提供的 905×588 原图，`image001-acn.png` 保留将中心文字 NGC 改为 ACN 的版本。当前页面使用 `image001-background.png`：通过内置图片编辑工具去除旧文字及边缘选框点，保留场景与面板，再由 `PptPresentation.jsx` 按原位置绘制六处 SVG 文字。文字使用标准字体、加大的字号和细描边，提高缩放时的清晰度及对比度；不对文字使用模糊或发光滤镜。图片与文字共用等比容器，浏览器原生缩放时同步对齐。

各次图片编辑的提示词记录在对应的 `.edit.json`；当前素材及文字来源记录在 `source.json`。底图随代码保存，重新生成 PPT 素材不会覆盖它或恢复图片中的旧文字。
