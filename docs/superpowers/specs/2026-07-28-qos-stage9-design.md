...
- 每条消息包含图片和文本。
- 图片固定占视频窗口宽度的 1/4，高度按图片比例自动缩放，不裁剪。
...
- 图片和文本的展示位置由 payload 中的 `imagePlacements[i]` 分别控制：`above/below` 表示 `images[i]` 相对 `dialogs[i]` 的上下关系，`left/right` 表示图片和文本组成的对话项整体显示在视频框左侧或右侧。
- 未配置时：
  - 页面不主动请求 `/api/v1/qos`。
