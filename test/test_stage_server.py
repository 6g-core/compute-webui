...
class WebUiApiStateQosCacheTest(unittest.TestCase):
    def test_dialog_images_are_cached_and_replayed_to_new_subscribers(self):
        ...
        state.publish_qos(dialog_payload("用户问题", placement=["left", "below"]), "dialogImages")

        subscriber = state.subscribe_qos()
        replayed = subscriber.get_nowait()

        self.assertEqual(["用户问题"], replayed["dialogs"])
        self.assertEqual([["left", "below"]], replayed["imagePlacements"])
    ...
    def test_validate_reset_payload(self):
        ...
        self.assertEqual("dialogImages", validate_qos_payload(dialog_payload("用户问题", placement=["right", "below"])))
        with self.assertRaisesRegex(ValueError, "only one left/right"):
            validate_qos_payload(dialog_payload("用户问题", placement=["left", "right"]))
