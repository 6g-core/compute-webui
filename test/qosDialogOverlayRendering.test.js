...
test("QoS conversation panel follows the supplied communication-log reference", () => {
  ...
  assert.match(panelSource, /const scrollAreaRef = useRef\(null\)/);
  assert.match(panelSource, /const \[scrollLocked, setScrollLocked\] = useState\(false\)/);
  assert.match(panelSource, /const bottomThreshold = 8/);
  assert.match(panelSource, /scrollArea\.scrollHeight - scrollArea\.scrollTop - scrollArea\.clientHeight <= bottomThreshold/);
  assert.match(panelSource, /setScrollLocked\(!isScrollAtBottom\(scrollArea\)\)/);
  assert.match(panelSource, /scrollArea\.scrollTop = scrollArea\.scrollHeight/);
  assert.match(panelSource, /min-h-0 flex-1 overflow-y-auto px-4 py-3/);
  assert.match(panelSource, /onScroll=\{handleScroll\}/);
  assert.match(panelSource, /onPointerDown=\{handlePointerDown\}/);
  assert.match(panelSource, /onWheel=\{handleManualScrollInput\}/);
  assert.match(panelSource, /items\.length \? items\.map/);
